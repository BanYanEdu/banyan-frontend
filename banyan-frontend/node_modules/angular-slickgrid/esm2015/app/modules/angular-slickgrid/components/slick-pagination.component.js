...
let SlickPaginationComponent = class SlickPaginationComponent {
    ...
    ngAfterViewInit() {
        ...
        if (this.dataView) {
            this.gridService.onItemAdded.subscribe((items) => this.onItemAddedOrRemoved(items, true));
            this.gridService.onItemDeleted.subscribe((items) => this.onItemAddedOrRemoved(items, false));
        }
    }
    ceil(number) {
        ...
        if (backendApi) {
            try {
                ...
                if (process instanceof Promise && process.then) {
                    process.then((processResult) => executeBackendProcessesCallback(startTime, processResult, backendApi, this._gridPaginationOptions));
                }
                else if (isObservable(process)) {
                    process.subscribe((processResult) => executeBackendProcessesCallback(startTime, processResult, backendApi, this._gridPaginationOptions), (error) => onBackendError(error, backendApi));
                }
            }
            catch (error) {
