...
import { Injectable, ComponentFactoryResolver, ApplicationRef, Injector, EventEmitter, Output, Input, Component, Inject, ElementRef, NgModule } from '@angular/core';
...
const dateEuroShortSorter = (value1, value2, sortDirection) => {
    return compareDates(value1, value2, FORMAT, sortDirection, true);
};

const FORMAT$1 = mapMomentDateFormatWithFieldType(FieldType.dateEuro);
const dateEuroSorter = (value1, value2, sortDirection) => {
    return compareDates(value1, value2, FORMAT$1, sortDirection, true);
};

const FORMAT$2 = mapMomentDateFormatWithFieldType(FieldType.dateIso);
 ...
 */
const compareObjects = (o1, o2, compareKey) => {
...
const FORMAT$5 = mapMomentDateFormatWithFieldType(FieldType.dateEuro);
const dateEuroFilterCondition = (options) => {
    const searchTerm = Array.isArray(options.searchTerms) && options.searchTerms[0] || '';
    if (searchTerm === null || searchTerm === '' || !moment$3(options.cellValue, FORMAT$5, true).isValid() || !moment$3(searchTerm, FORMAT$5, true).isValid()) {
        return false;
    }
    const dateCell = moment$3(options.cellValue, FORMAT$5, true);
    const dateSearch = moment$3(searchTerm, FORMAT$5, true);
...
const FORMAT$6 = mapMomentDateFormatWithFieldType(FieldType.dateEuroShort);
const dateEuroShortFilterCondition = (options) => {
    const searchTerm = Array.isArray(options.searchTerms) && options.searchTerms[0] || '';
    if (searchTerm === null || searchTerm === '' || !moment$4(options.cellValue, FORMAT$6, true).isValid() || !moment$4(searchTerm, FORMAT$6, true).isValid()) {
        return false;
    }
    const dateCell = moment$4(options.cellValue, FORMAT$6, true);
    const dateSearch = moment$4(searchTerm, FORMAT$6, true);
    // run the filter condition with date in Unix Timestamp format
    return testFilterCondition(options.operator || '==', parseInt(dateCell.format('X'), 10), parseInt(dateSearch.format('X'), 10));
};
...
const dateFilterCondition = (options) => {
    ...
    const dateCell = moment$5(options.cellValue);
    const dateSearch = moment$5(searchTerm);
    // run the filter condition with date in Unix Timestamp format
    return testFilterCondition(options.operator || '==', parseInt(dateCell.format('X'), 10), parseInt(dateSearch.format('X'), 10));
};
...
const FORMAT$7 = mapMomentDateFormatWithFieldType(FieldType.dateIso);
const dateIsoFilterCondition = (options) => {
    ...
    const dateCell = moment$6(options.cellValue, FORMAT$7, true);
    const dateSearch = moment$6(searchTerm, FORMAT$7, true);
    // run the filter condition with date in Unix Timestamp format
    return testFilterCondition(options.operator || '==', parseInt(dateCell.format('X'), 10), parseInt(dateSearch.format('X'), 10));
};
...
const FORMAT$8 = mapMomentDateFormatWithFieldType(FieldType.dateUs);
const dateUsFilterCondition = (options) => {
    ...
    const dateCell = moment$7(options.cellValue, FORMAT$8, true);
    const dateSearch = moment$7(searchTerm, FORMAT$8, true);
    // run the filter condition with date in Unix Timestamp format
    return testFilterCondition(options.operator || '==', parseInt(dateCell.format('X'), 10), parseInt(dateSearch.format('X'), 10));
};
...
const FORMAT$9 = mapMomentDateFormatWithFieldType(FieldType.dateUsShort);
const dateUsShortFilterCondition = (options) => {
    ...
    const dateCell = moment$8(options.cellValue, FORMAT$9, true);
    const dateSearch = moment$8(searchTerm, FORMAT$9, true);
    // run the filter condition with date in Unix Timestamp format
    return testFilterCondition(options.operator || '==', parseInt(dateCell.format('X'), 10), parseInt(dateSearch.format('X'), 10));
};
...
const dateUtcFilterCondition = (options) => {
    ...
    const dateCell = moment$9(options.cellValue, moment$9.ISO_8601, true);
    const dateSearch = moment$9(searchTerm, searchDateFormat, true);
    // run the filter condition with date in Unix Timestamp format
    return testFilterCondition(options.operator || '==', parseInt(dateCell.format('X'), 10), parseInt(dateSearch.format('X'), 10));
};
            ...
            if (gridOptions && gridOptions.rowDetailView) {
                if (!this._addon) {
                    ...
                    if (!gridOptions.rowDetailView.postTemplate) {
                        ...
                        gridOptions.rowDetailView.postTemplate = (itemDetail) => DOMPurify$1.sanitize(`<div class="${ROW_DETAIL_CONTAINER_PREFIX}${itemDetail.id}"></div>`);
                    }
...
const moment$a = moment_; // patch to fix rollup "moment has no default export" issue, document here https://github.com/rollup/rollup/issues/670
require('flatpickr');
/*
...
class DateEditor {
    ...
    serializeValue() {
        ...
        const value = moment$a(domValue).format(outputFormat);
        return value;
    }
    applyValue(item, state) {
        const fieldName = this.columnDef && this.columnDef.field;
        ...
        // when it's a complex object, then pull the object name only, e.g.: "user.firstName" => "user"
        const fieldNameFromComplexObject = fieldName.indexOf('.') ? fieldName.substring(0, fieldName.indexOf('.')) : '';
        const newValue = state ? moment$a(state, outputFormat).toDate() : '';
        const validation = this.validate(newValue);
        item[fieldNameFromComplexObject || fieldName] = (validation && validation.valid) ? newValue : '';
    }
    isValueChanged() {
...
const moment$b = moment_; // patch to fix rollup "moment has no default export" issue, document here https://github.com/rollup/rollup/issues/670
/**
...
function getAssociatedDateFormatter(fieldType, defaultSeparator) {
    ...
    return (row, cell, value, columnDef, dataContext, grid) => {
        ...
        const isDateValid = moment$b(value, defaultDateFormat, false).isValid();
        let outputDate = (value && isDateValid) ? moment$b(value).format(defaultDateFormat) : value;
...
export { Aggregators, AngularSlickgridComponent, AngularSlickgridModule, AngularUtilService, AutoTooltipExtension, CaseType, CellExternalCopyManagerExtension, CheckboxSelectorExtension, CollectionService, ColumnPickerExtension, DelimiterType, DraggableGroupingExtension, Editors, EmitterType, ExportService, ExtensionName, ExtensionService, ExtensionUtility, FieldType, FileType, FilterConditions, FilterFactory, FilterMultiplePassType, FilterService, Filters, Formatters, GraphqlService, GridEventService, GridMenuExtension, GridOdataService, GridService, GridStateService, GridStateType, GroupItemMetaProviderExtension, GroupTotalFormatters, GroupingAndColspanService, HeaderButtonExtension, HeaderMenuExtension, KeyCode, OdataService, OperatorType, ResizerService, RowDetailViewExtension, RowMoveManagerExtension, RowSelectionExtension, SharedService, SlickPaginationComponent, SlickgridConfig, SortDirection, SortDirectionNumber, SortService, Sorters, addToArrayWhenNotExists, addWhiteSpaces, castToPromise, charArraysEqual, decimalFormatted, executeBackendProcessesCallback, findOrDefault, formatNumber, getDescendantProperty, getScrollBarWidth, htmlDecode, htmlEncode, htmlEntityDecode, htmlEntityEncode, mapFlatpickrDateFormatWithFieldType, mapMomentDateFormatWithFieldType, mapOperatorByFieldType, mapOperatorType, onBackendError, parseBoolean, parseUtcDate, sanitizeHtmlToText, titleCase, toCamelCase, toKebabCase, toSnakeCase, uniqueArray, uniqueObjectArray, unsubscribeAllObservables, AvgAggregator as ɵa, MinAggregator as ɵb, stringFilterCondition as ɵba, testFilterCondition as ɵbb, AutoCompleteFilter as ɵbc, CompoundDateFilter as ɵbd, CompoundInputFilter as ɵbe, CompoundInputNumberFilter as ɵbf, CompoundInputPasswordFilter as ɵbg, CompoundSliderFilter as ɵbh, InputFilter as ɵbi, InputMaskFilter as ɵbj, InputNumberFilter as ɵbk, InputPasswordFilter as ɵbl, MultipleSelectFilter as ɵbm, SelectFilter as ɵbn, NativeSelectFilter as ɵbo, SingleSelectFilter as ɵbp, SliderFilter as ɵbq, arrayObjectToCsvFormatter as ɵbr, arrayToCsvFormatter as ɵbs, boldFormatter as ɵbt, checkboxFormatter as ɵbu, checkmarkFormatter as ɵbv, complexObjectFormatter as ɵbw, collectionFormatter as ɵbx, collectionEditorFormatter as ɵby, getAssociatedDateFormatter as ɵbz, MaxAggregator as ɵc, deleteIconFormatter as ɵca, decimalFormatter as ɵcb, dollarFormatter as ɵcc, dollarColoredFormatter as ɵcd, dollarColoredBoldFormatter as ɵce, editIconFormatter as ɵcf, hyperlinkFormatter as ɵcg, iconFormatter as ɵch, infoIconFormatter as ɵci, italicFormatter as ɵcj, lowercaseFormatter as ɵck, maskFormatter as ɵcl, multipleFormatter as ɵcm, percentFormatter as ɵcn, percentCompleteFormatter as ɵco, percentCompleteBarFormatter as ɵcp, percentSymbolFormatter as ɵcq, progressBarFormatter as ɵcr, translateFormatter as ɵcs, translateBooleanFormatter as ɵct, uppercaseFormatter as ɵcu, yesNoFormatter as ɵcv, avgTotalsFormatter as ɵcw, avgTotalsDollarFormatter as ɵcx, avgTotalsPercentageFormatter as ɵcy, maxTotalsFormatter as ɵcz, SumAggregator as ɵd, minTotalsFormatter as ɵda, sumTotalsFormatter as ɵdb, sumTotalsBoldFormatter as ɵdc, sumTotalsColoredFormatter as ɵdd, sumTotalsDollarFormatter as ɵde, sumTotalsDollarBoldFormatter as ɵdf, sumTotalsDollarColoredFormatter as ɵdg, sumTotalsDollarColoredBoldFormatter as ɵdh, dateSorter as ɵdi, dateIsoSorter as ɵdj, dateEuroSorter as ɵdk, dateEuroShortSorter as ɵdl, dateUsSorter as ɵdm, dateUsShortSorter as ɵdn, numericSorter as ɵdo, objectStringSorter as ɵdp, stringSorter as ɵdq, AutoCompleteEditor as ɵe, CheckboxEditor as ɵf, DateEditor as ɵg, FloatEditor as ɵh, IntegerEditor as ɵi, LongTextEditor as ɵj, MultipleSelectEditor as ɵk, SelectEditor as ɵl, SingleSelectEditor as ɵm, SliderEditor as ɵn, TextEditor as ɵo, executeMappedCondition as ɵp, booleanFilterCondition as ɵq, collectionSearchFilterCondition as ɵr, dateEuroFilterCondition as ɵs, dateEuroShortFilterCondition as ɵt, dateFilterCondition as ɵu, dateIsoFilterCondition as ɵv, dateUtcFilterCondition as ɵw, dateUsFilterCondition as ɵx, dateUsShortFilterCondition as ɵy, numberFilterCondition as ɵz };
